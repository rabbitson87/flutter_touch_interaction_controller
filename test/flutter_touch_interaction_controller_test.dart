...
void main() {
  ...
  const String isAccessibilityPermissionEnabled =
      '${prefixName}isAccessibilityPermissionEnabled';
  const String requestAccessibilityPermission =
      '${prefixName}requestAccessibilityPermission';
  const String touch = '${prefixName}touch';

  setUp(() {
    binaryMessenger.setMockMessageHandler(isAccessibilityPermissionEnabled,
        (ByteData? message) async => codec.encodeMessage(<Object?>[true]));
    binaryMessenger.setMockMessageHandler(requestAccessibilityPermission,
        (ByteData? message) async => codec.encodeMessage(<Object?>[true]));
    binaryMessenger.setMockMessageHandler(touch,
        (ByteData? message) async => codec.encodeMessage(<Object?>[true]));
  });

  tearDown(() {
    binaryMessenger.setMockMessageHandler(
        isAccessibilityPermissionEnabled, null);
  ...
  test('touch', () async {
    expect(await FlutterTouchInteractionController.touch(Point(x: 100, y: 100)),
        true);
  });
